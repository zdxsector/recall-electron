...
export default class ExportMarkdown {
  ...
  constructor(
    { listIndentation }: IExportMarkdownOptions = {
      listIndentation: 1,
    }
  ) {
    ...
    for (const state of states) {
      ...
      switch (state.name) {
        ...
        case 'task-list': {
          ...
          if (insertNewLine) this.insertLineBreak(result, indent);
        ...
        case 'task-list-item': {
          ...
          if (loose) this.insertLineBreak(result, indent);
  ...
  serializeTable(state: ITableState, indent: string) {
    ...
    tableData.forEach((r, i) => {
      ...
      result.push(rs);
      if (i === 0) {
        const cutOff = `${indent}|${columnWidth
          .map(({ width, align }) => {
