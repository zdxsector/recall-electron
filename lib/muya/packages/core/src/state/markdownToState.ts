    ...
    while ((token = tokens.shift())) {
      switch (token.type) {
        ...
        case 'heading': {
          ...
          if (name === 'setext-heading') meta.underline = marker;
        ...
        case 'table': {
          ...
          state.children.push({
            name: 'table.row',
            children: header.map((h: { text: string }, i: string | number) => ({
              // TODO: @jocs fix type
              name: 'table.cell',
        ...
        case 'list_item': {
          ...
          state = {
            name: (listItemType === 'task'
              ? 'task-list-item'
              : 'list-item') as any,
            children: [],
          };

          if (listItemType === 'task') (state as any).meta = { checked };
