...
export function parseSrcAndTitle(text = '') {
  ...
  if (title) src = text.substring(0, text.length - rawTitle.length).trim();
  else src = text.trim();
...
export function validateEmphasize(
  src: string,
  offset: number,
  marker: string,
  pending: string,
  ...
  if (!canOpenEmphasis(src, marker, pending)) return false;

  if (!canCloseEmphasis(src, offset, marker)) return false;
