...
export class CodeBlockLanguageSelector extends BaseScrollFloat {
  ...
  render() {
    ...
    let children = (
      ...
      let iconClassNames;
      if (item.name) iconClassNames = fileIcons.getClassByLanguage(item.name);
      ...
      if (iconClassNames) {
        const iconSelector = `span${iconClassNames
          .split(/\s/)
          .map((s: string) => `.${s}`)
          .join('')}`;
    ...
    if (children.length === 0) children = [h('div.no-result', 'No result')];
