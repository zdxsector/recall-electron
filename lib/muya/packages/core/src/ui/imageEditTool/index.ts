...
export class ImageEditTool extends BaseFloat {
  ...
  private _handleEnter(event: Event) {
    ...
    if (event.key === EVENT_KEYS.Enter) this._handleConfirm();
  }
  ...
  private async _replaceImageWithUpload(
    ...
    if (localPath) {
      this.muya.editor.inlineRenderer.renderer.urlMap.set(
        uploadedSrc,
        localPath
      );
    }
  ...
  private async _handleMoreClick() {
    if (!this.options.imagePathPicker) return;
  ...
  private _render() {
    const {
      _oldVNode: oldVNode,
      _imageSelectorContainer: imageSelectorContainer,
      _state: { src },
    } = this;
