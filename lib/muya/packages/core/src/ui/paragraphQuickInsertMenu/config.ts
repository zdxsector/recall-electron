...
export function replaceBlockByLabel({
  ...
  switch (label) {
    ...
    case 'block-quote':
      ...
      if (label === 'paragraph') (state as IParagraphState).text = text;
      else if (label === 'block-quote')
        ((state as IBlockQuoteState).children[0] as IParagraphState).text =
          text;
    ...
    case 'order-list':
      ...
      if (text) state.children[0].children[0].text = text;
