...
function parseSelector(str = '') {
  ...
  for (const tagName of HTML_TAGS) {
    if (
      ...
      tag = tagName;
      if (VOID_HTML_TAGS.includes(tagName as any)) isVoid = true;
...
class CodeBlockContent extends Content {
  ...
  constructor(muya: Muya, state: CodeContentState) {
    ...
    if (hasStateMeta(state)) this.initialLang = state.meta.lang;
    else this.initialLang = LANG_HASH[state.name];
  ...
  override update(_cursor: ICursor, highlights = []) {
    ...
    if (
      ...
      try {
        ...
        // Fallback to plain code if Prism fails (e.g., language dependency not loaded)
        console.warn(`Prism highlighting failed for "${fullLengthLang}":`, err);
    ...
    if (start.offset === end.offset) {
      ...
      if (
        ...
        try {
          ...
          // Ignore tokenize errors - Firefox compatibility code is best-effort
          console.warn(`Prism tokenize failed for "${lang}":`, err);
        }
      }
    }
  }
